...
import { CircleDot, MessageCircle, Tag } from 'lucide-vue-next'
...
const summaryItems = computed(() => [
  {
    id: 'repository',
    icon: CircleDot,
    label: t('issue.summary.repository'),
    value: repository.value,
  },
  {
    id: 'labels',
    icon: Tag,
    label: t('issue.summary.labels'),
    value: t('issue.values.placeholder'),
  },
  {
    id: 'comments',
    icon: MessageCircle,
    label: t('issue.summary.comments'),
    value: t('issue.values.placeholder'),
  },
])
</script>
    ...
    <div class="mx-auto grid w-full max-w-5xl gap-5 px-6 py-6">
      <div class="grid max-w-3xl gap-2">
        ...
        <p class="max-w-2xl text-label text-muted-foreground">
          {{ t('issue.description', { repository, number }) }}
        </p>
      </div>
      ...
      <div class="grid gap-2 lg:grid-cols-[minmax(0,1.4fr)_minmax(18rem,0.8fr)]">
        <div class="grid gap-2 rounded-lg border border-border bg-card p-3">
          <div class="text-label font-medium text-foreground">
            {{ t('issue.sections.overview.title') }}
          </div>
          <p class="text-body text-muted-foreground">
            {{ t('issue.sections.overview.description') }}
          </p>
        </div>

        <div class="grid gap-2 rounded-lg border border-border bg-card p-3">
          <div class="text-label font-medium text-foreground">
            {{ t('issue.sections.activity.title') }}
          </div>
          <p class="text-body text-muted-foreground">
            {{ t('issue.sections.activity.description') }}
          </p>
        </div>
      </div>
    </div>
