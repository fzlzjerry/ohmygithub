...
import { GitPullRequest, Inbox, ListChecks } from 'lucide-vue-next'
...
const categoryKey = computed(() => props.tab.pullRequestCategory ?? 'inbox')
const categoryLabel = computed(() => t(`workspace.sidebar.pullRequestCategories.${categoryKey.value}`))

const summaryItems = computed(() => [
  {
    id: 'category',
    icon: GitPullRequest,
    label: t('pullRequestList.summary.category'),
    value: categoryLabel.value,
  },
  {
    id: 'source',
    icon: Inbox,
    label: t('pullRequestList.summary.source'),
    value: t(`pullRequestList.sources.${categoryKey.value}`),
  },
  {
    id: 'status',
    icon: ListChecks,
    label: t('pullRequestList.summary.status'),
    value: t('pullRequestList.values.placeholder'),
  },
])
</script>
      ...
      <div class="grid gap-2 lg:grid-cols-[minmax(0,1.4fr)_minmax(18rem,0.8fr)]">
        <div class="grid gap-2 rounded-lg border border-border bg-card p-3">
          <div class="select-none text-label font-medium text-foreground">
            {{ t('pullRequestList.sections.overview.title') }}
          </div>
          <p class="text-body text-muted-foreground">
            {{ t('pullRequestList.sections.overview.description') }}
          </p>
        </div>

        <div class="grid gap-2 rounded-lg border border-border bg-card p-3">
          <div class="select-none text-label font-medium text-foreground">
            {{ t('pullRequestList.sections.sidebar.title') }}
          </div>
          <p class="text-body text-muted-foreground">
            {{ t('pullRequestList.sections.sidebar.description') }}
          </p>
        </div>
      </div>
    </div>
